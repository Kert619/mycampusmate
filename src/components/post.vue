<template>
  <div class="p-3 rounded bg-white position-relative">
    ...
    >
      <button class="btn" @click="deletePost" v-if="!loading">
        <svg
      ...
      <span class="text-danger" v-else>Deleting....</span>
    </div>
    ...
    >
      ...
      <button class="btn d-flex align-items-center gap-2" @click="toggleLike">
        ...
        <span
          >({{
            JSON.parse(post.likes)?.length > 0
    ...
    <div v-show="showCreateComment">
      ...
      <div class="d-flex flex-column gap-2">
        <Comment
          v-for="comment in post.comments_to_post"
          :fullname="
...
const toggleLike = async () => {
  ...
  emits("toggleLike", props.post.id);
};
